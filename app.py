...
def job():
    ...
    if not tasks.empty():
        ...
        try:
            ...
            payload = {
                "status": "SUCCESS",
                "uri": "http://localhost:5001/v1/uploads/{}".format(filename)
            }
...
def upload():
    ...
    output_file_name = "{}/BRCA2_data_mutations_extended-{}.maf".format(app.config['UPLOAD_FOLDER'], datetime.datetime.now().isoformat())
    runPreProcessamento(file, columns, output_file_name)

    tasks.put({
        "execution_id": execution_id,
        "step_id": step_id,
        "filename": output_file_name.replace("static/", "")
    })

    return jsonify({
        "urlToCheck": "http://localhost:5001/v1/pipegene/{}/status".format(step_id),
        "message": "IN_PROGRESS",
    })
